...
class AgentTrustRecord:
    ...
    def get_trust(
        ...
        strict_domain=True enforces the corrected-protocol invariant: trust in an
        unseen domain is the cold-start prior. The legacy ``_global`` fallback is
        only used when strict_domain=False (backward compatibility).
        """
...
class TrustManager:
    ...
    def get_trust(
        ...
        *,
        ...
        strict_domain=True enforces domain isolation (no implicit _global fallback).
        """
    ...
    def seed_from_counts(
        ...
        """Hydrate a single (agent, domain) trust record from durable counts.

        Used by the corrected-protocol SQLite trust ledger to restore historical
        trust on service restart. Setting the counters directly (rather than
        replaying individual outcomes) keeps restart hydration exact and
        idempotent.
        """
        if agent_id not in self._records:
            self._records[agent_id] = AgentTrustRecord(agent_id=agent_id)
        rec = self._records[agent_id]
        dr = rec.domains.setdefault(domain, AgentDomainRecord())
        dr.verified_correct = int(confirmed_correct)
        dr.verified_wrong = int(confirmed_incorrect)
        dr.total_claims = dr.verified_correct + dr.verified_wrong
        dr.last_outcome_time = last_verified_at
    def get_outcome_counts(
        ...
        strict_domain=True returns zero counts for unseen domains instead of
        falling back to the _global aggregate.
        """
        if agent_id not in self._records:
            return {"total": 0, "correct": 0, "incorrect": 0}
        record = self._records[agent_id]
        domain_rec = record.domains.get(domain)
        if not domain_rec and not strict_domain:
            domain_rec = record.domains.get("_global")
        if not domain_rec:
            return {"total": 0, "correct": 0, "incorrect": 0}
        return {
    ...
    def get_trust_with_meta(
        ...
        strict_domain=True enforces domain isolation (no _global fallback).
        """
        counts = self.get_outcome_counts(agent_id, domain=domain, strict_domain=strict_domain)
        trust_score = self.get_trust(
            agent_id, domain=domain, current_time=current_time, strict_domain=strict_domain
        )
        return {
