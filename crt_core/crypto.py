...
The CRT middleware verifies that signature against the provider's Ed25519
...
Signing message (canonical, V1)
-------------------------------
::

    crt-evidence-binding/2|provider_id|key_id|evidence_type|source_id|evidence_hash|assertion_hash|issued_at|expires_at|nonce
...
------------------------
* Set ``CRT_EVIDENCE_PUBLIC_KEY`` to a hex/base64 Ed25519 public key for production.
* Set ``CRT_ALLOW_DEV_EVIDENCE_KEY=1`` to permit the built-in development fallback key.
...
_DEV_SEED = hashlib.sha256(
    b"CRT development evidence-provider Ed25519 seed v1 (dev-only, NOT for production)"
).digest()
...
# SQLite-backed guard (crt_service.storage.SQLiteReplayGuard) for durability.
...
def _load_public_key(provider_id: Optional[str] = None, key_id: str = "current") -> Ed25519PublicKey:
    ...
    Priority:
      ...
      2. ``CRT_EVIDENCE_PUBLIC_KEY`` env var (hex or base64, 32 raw bytes)
      3. Dev fallback key (ONLY if CRT_ALLOW_DEV_EVIDENCE_KEY=1)
    ...
    raw = os.environ.get("CRT_EVIDENCE_PUBLIC_KEY")
    if raw:
        ...
        if data is None:
            ...
            raise EvidenceKeyConfigurationError(
                "CRT_EVIDENCE_PUBLIC_KEY is set but is not a valid Ed25519 public key."
            )
        try:
    ...
    if os.environ.get("CRT_ALLOW_DEV_EVIDENCE_KEY") == "1":
        return _dev_private_key().public_key()

    # No valid key available - fail closed
    raise EvidenceKeyConfigurationError(
        "No valid evidence public key configured. "
        "Set CRT_EVIDENCE_PUBLIC_KEY, or CRT_ALLOW_DEV_EVIDENCE_KEY=1 for dev/test."
    )
...
_MESSAGE_PREFIX = "crt-evidence-binding/2"


def canonical_assertion_hash(
    agent_id: str,
    ...
    *,
    ...
    """Hash one exact operational assertion for every supported write path."""
    if not isinstance(agent_id, str) or not agent_id.strip():
        raise ValueError("agent_id is required for assertion binding")
    if not isinstance(assertion_payload, dict) or not assertion_payload:
        raise ValueError("non-empty assertion_payload is required for assertion binding")
    if isinstance(timestamp, str):
        text = timestamp.strip().replace("Z", "+00:00")
        if not text:
            raise ValueError("timestamp is required for assertion binding")
    ...
    return canonical_sha256({
        "binding_version": "crt-assertion/1",
        "agent_id": agent_id,
...
def _compute_assertion_hash(agent_id: str, timestamp: str, assertion_payload: dict) -> str:
    """Compatibility alias using the mandatory bound representation."""
    return canonical_assertion_hash(agent_id, timestamp, assertion_payload)
...
def build_evidence_message(
    ...
    """
    Build the canonical byte message signed/verified for evidence binding (V1).
...
def verify_evidence_signature_crypto(
    ...
    Ed25519 signature over the canonical V1 evidence message, verifiable against
    the configured public key AND temporally valid at ``reference_time``.

    Temporal enforcement (Phase 4): the V1 message binds ``issued_at`` and
...
def sign_evidence_message(
    ...
    """
    Produce a base64 Ed25519 signature over the canonical V1 evidence message,
...
def sign_assertion_evidence(
    ...
    *,
    ...
    """Development/test signer bound to the exact operational assertion."""
    return sign_evidence_message(
        evidence_type, source_id, content_hash=content_hash,
        assertion_hash=canonical_assertion_hash(
            agent_id, timestamp, assertion_payload, domain=domain
        ),
        provider_id=provider_id, key_id=key_id, issued_at=issued_at,
...
def sign_evidence_message_with_key(
    ...
    """
    Sign the canonical V1 evidence message with a SPECIFIC Ed25519 private key.
...
def sign_evidence_for_records(records: List[EvidenceRecord], assertion_hash: Optional[str] = None) -> Optional[str]:
    ...
    :func:`crt_core.provenance.validate_and_stamp`.
...
def benchmark_dev_evidence_key():
    ...
    Sets ``CRT_ALLOW_DEV_EVIDENCE_KEY=1`` only when ``CRT_EVIDENCE_PUBLIC_KEY``
    ...
    Example::
        ...
        from crt_core.crypto import benchmark_dev_evidence_key
        with benchmark_dev_evidence_key():
            results = run_benchmark(...)
    """
    env_key = "CRT_ALLOW_DEV_EVIDENCE_KEY"
    had_real = bool(os.environ.get("CRT_EVIDENCE_PUBLIC_KEY"))
