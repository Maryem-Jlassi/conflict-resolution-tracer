"""Global event bus — CRT Core publishes; Inspector subscribes.

Design rules:
  • CRT Core never imports from the inspector layer.
...
class EventBus:
    ...
    Usage
    -----
    # Publisher (crt_core internals):
        bus = get_event_bus()
        bus.publish(CRTEvent(EventType.CONFLICT_RESOLVED, datetime.utcnow(), {...}))
    ...
    def publish(self, event: LCMEvent) -> None:
        ...
        Inspector problems cannot propagate back into CRT Core.
        """
