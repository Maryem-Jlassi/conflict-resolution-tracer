...
class LCMClient:
    """
    Framework-agnostic client for Living Context Memory service.

    Uses httpx only - no dependency on lcm_core or agent frameworks.
    """
    ...
    def verify(self, agent_id: str, correct: bool, domain: str = "_global") -> Dict[str, Any]:
        """
        ...
        with httpx.Client(timeout=self.timeout) as client:
            response = client.post(
                ...
                json={"agent_id": agent_id, "correct": correct, "domain": domain},
            )
            response.raise_for_status()
            return response.json()
    ...
    def health_check(self) -> Dict[str, Any]:
        """Check if LCM service is operational."""
        with httpx.Client(timeout=self.timeout) as client:
