"""
LCM Write Pipeline - single orchestrator that connects all lcm_core modules.
...
def _check_llm_contamination():
    """
    Check for LLM-related imports that were loaded *by the lcm_core package itself*.

    We only reject contamination if lcm_core code directly caused the import.
    ...
    is allowed to use LLMs.  Only direct imports *inside* lcm_core modules are
    forbidden.
    """
    import traceback as _tb
    # Walk the call stack looking for frames inside lcm_core
    # If we find a frame in lcm_core that imported a forbidden module, that's contamination.
    # In practice, we check whether any lcm_core module is present in the *importer*
    # of a forbidden module — i.e. whether a forbidden module's __spec__.parent is lcm_core.
    for mod_name, mod in list(sys.modules.items()):
        if mod is None:
            continue
        if not any(f in mod_name for f in _FORBIDDEN_LLM_MODULES):
            continue
        # Forbidden module is present. Check if it was imported FROM lcm_core.
        spec = getattr(mod, "__spec__", None)
        if spec is not None and spec.parent and "lcm_core" in spec.parent:
            raise RuntimeError(
                f"LLM contamination detected: '{mod_name}' was imported inside lcm_core. "
                f"LCM middleware must be zero-LLM. LLM calls are only permitted in agent layers."
            )
        # Also reject if any lcm_core module directly references the forbidden package
        ...
        # their __spec__.parent is outside lcm_core — that is fine.
...
class WritePipeline:
    ...
    def __init__(
        ...
        # Use DEFAULT_CONFIG.psi_weights if no conflict_engine provided
        if conflict_engine is None:
            ...
            resolution_config = ResolutionConfig(
                ...
                w_provenance=psi_config.w_provenance,
    ...
    async def _process_inner(
        ...
        *,
        ...
        try:
            ...
            _emit(LCMEvent(EventType.WRITE_REJECTED, datetime.utcnow(), {
        ...
        from lcm_core.config import (
            TRUST_REJECT_THRESHOLD,
            HIGH_CONFIDENCE_UNTRUSTED_THRESHOLD,
            LOW_TRUST_THRESHOLD,
        )
        
        trust = self.trust.get_trust(stamped.agent_id, domain)
        ...
        _emit(LCMEvent(EventType.PROVENANCE_VALIDATED, datetime.utcnow(), {
            "agent_id":            stamped.agent_id,
        ...
        if loop_result and loop_result.is_looping:
            _emit(LCMEvent(EventType.LOOP_DETECTED, datetime.utcnow(), {
                "path":             path,
                "agent_id":         stamped.agent_id,
        ...
        try:
            result = await self._locked_write(stamped, path, domain, reference_time)
        finally:
    ...
    async def _locked_write(
        ...
        if existing is None:
            ...
            _emit(LCMEvent(EventType.MEMORY_INGESTED, datetime.utcnow(), {
                "path":                path,
                "agent_id":            stamped.agent_id,
                "session_id":          stamped.session_id,
        ...
        _emit(LCMEvent(EventType.CONFLICT_DETECTED, datetime.utcnow(), {
            "path":           path,
        ...
        ref_time = reference_time if reference_time is not None else datetime.utcnow()
        t_existing = self.trust.get_trust(existing.agent_id, effective_domain)
        t_incoming = self.trust.get_trust(stamped.agent_id, effective_domain)
        ...
        psi_components = ["R", "C", "T", "A"]
        component_deltas = {}
        for comp in psi_components:
            w_key = "w_" + comp.lower()
            w_weight = breakdown_winner.get(w_key, 0.25)
            l_weight = breakdown_loser.get(w_key, 0.25)
        ...
        if conflict_result.unresolved:
            ...
            _emit(LCMEvent(EventType.CONFLICT_UNRESOLVED, datetime.utcnow(), conflict_event_data))
            record_failure(
        ...
        _emit(LCMEvent(EventType.CONFLICT_RESOLVED, datetime.utcnow(), conflict_event_data))
    ...
    def record_verification(
        ...
        _emit(LCMEvent(EventType.TRUST_UPDATED, operation_time, {
            "agent_id":        agent_id,
            "correct":         correct,
            "domain":          domain,
