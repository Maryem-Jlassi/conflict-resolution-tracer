...
from .crypto import verify_evidence_signature_crypto
...
def verify_evidence_signature(
    ...
    (``LCM_EVIDENCE_PUBLIC_KEY`` env var, or the dev fallback key) AND still
...
def validate_and_stamp(
    ...
    if records:
        verified_confidence = _confidence_engine.calculate(
            records,
        ...
        agreeing_agents
        or total_independent_agents
        or verified_memories_consistent is not None
    ):
        verified_confidence = _confidence_engine.calculate(
            [],
            agreeing_agents=agreeing_agents,
