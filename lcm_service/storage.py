"""SQLite-backed storage for LCM packets."""
...
from lcm_core.replay import ReplayGuard, nonce_fingerprint
...
class SQLiteStorage:
        ...
        with self._get_connection() as conn:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS umf_packets (
                    ...
                    agent_confidence_score REAL NOT NULL,
            ...
            """)
            # Backward compatibility: migrate old columns
            try:
                conn.execute("ALTER TABLE umf_packets ADD COLUMN agent_confidence_score REAL")
                conn.execute("DROP INDEX IF EXISTS idx_confidence")
            except Exception:
                pass  # Column already exists or table doesn't exist yet
            try:
                conn.execute("ALTER TABLE umf_packets ADD COLUMN provenance_info TEXT")
            except Exception:
                pass  # Column already exists
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_path 
                ON umf_packets(path, is_archived)
            """)
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_agent_session
                ON umf_packets(agent_id, session_id)
            """)
            # Replay-protection table (Phase 5): unique nonce fingerprints.
            conn.execute("""
            ...
            """)
            # Evidence-provider key registry (Phase 6): persisted, auditable.
            conn.execute("""
            ...
            """)
            # Provenance lineage (Phase 8): first-class derivation chain nodes.
            conn.execute("""
    ...
    def store_lineage_node(self, node: Any) -> None:
        ...
        from lcm_core.lineage import LineageNode
...
class SQLiteProviderRegistry:
    ...
    Mirrors the interface of ``lcm_core.crypto.EvidenceProviderRegistry``
...
class SQLiteLineageStore:
    ...
    :class:`lcm_core.lineage.LineageStore` protocol against the
